...
#Se o conda não estiver instalado, instalar. Se tiver, instalar o canu.
if ! command -v conda &> /dev/null; then
    echo "Conda não está instalado. Instalando-o..."
    wget https://repo.anaconda.com/archive/Anaconda3-2024.06-1-Linux-x86_64.sh
...
echo "Atenção: o script deve ser executado no diretório em que se encontram as leituras."
...
read -p "Qual o seu tipo de leituras (-pacbio|-nanopore|-pacbio-hifi)? " long
...
#execução canu
canu -p "$prefix" -d "$saida" genomeSize="$size" maxMemory="$memory" maxThreads="$threads" -"$long" "$input"
